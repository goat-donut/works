...
def ig(y, left, right):
    n = len(y)
    return gini(y) - gini(left)*(len(left)/n) - gini(right)*(len(right)/n)
IG = -1
th = None
fj = None
n_features = x_train.shape[1]

for j in range(n_features):  # перебор признаков
    range_t = np.arange(min(x_train[:, j])+0.1, max(x_train[:, j])-0.1, 0.1)
    for t in range_t:  # перебор порогов
        left = y_train[x_train[:, j] <= t]
        right = y_train[x_train[:, j] > t]
        gain = ig(y_train, left, right)
        if gain > IG:
            IG = gain
            th = t
            fj = j