...
# Данные (пример небольшой, заменяй на свой полный)
x_train = np.array(data_x)
y_train = np.array(data_y)

# Диапазон порогов для первого признака
...
def gini(y):
    if len(y) == 0:
        return 0
    y01 = (y == 1).astype(int)  # -1 → 0, 1 → 1
    p = np.bincount(y01) / len(y01)
    return 1 - np.sum(p**2)
...
for t in range_t:
    left = y_train[x_train[:, 0] <= t]
    right = y_train[x_train[:, 0] > t]
    gain = info_gain(y_train, left, right)
    if gain > IG:
        IG, t0 = gain, t
